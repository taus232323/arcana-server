...
pub fn build(router: Router<State>, server: &Server) -> Router<State> {
	...
	let mut router = router
		...
		.route(
			"/_matrix/client/v3/account/password/email/reset",
			post(client::reset_password_via_email_route),
		)
		...
		.route("/api/invite", post(client::arcana_invite::create_invite))
		.route("/api/invite/{token}", get(client::arcana_invite::get_invite))
		.route("/api/invite/{token}", axum::routing::delete(client::arcana_invite::revoke_invite))
